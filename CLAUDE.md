...
- **Разбивать коммиты по семантике**: если изменения затрагивают несколько независимых вещей — делать несколько коммитов. Каждый коммит должен компилироваться отдельно
- **Отдельные фичи — в отдельных worktree**: каждая ветка разрабатывается в своём worktree в `.claude/worktrees/`
